...
export default {
  ...
  methods: {
    ...
    async fetchFilterBarData() {
      try {
        const response = await axios.get('/api/referenceBooks/')
    ...
    async fetchNoticeDetails() {
      try {
        const response = await axios.get(`/api/animal/15`)
        ...
        if (response.data.photo.first) this.photos[0] = `/static/${response.data.photo.first}`
        if (response.data.photo.second) this.photos[1] = `/static/${response.data.photo.second}`
        if (response.data.photo.third) this.photos[2] = `/static/${response.data.photo.third}`
        if (response.data.photo.fourth) this.photos[3] = `/static/${response.data.photo.fourth}`
        if (this.photos.length === 0) this.photos[0] = '/static/noPhoto.svg'
